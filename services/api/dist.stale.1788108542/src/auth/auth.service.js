...
const login_lockout_service_1 = require("./login-lockout.service");
...
let AuthService = class AuthService {
    ...
    mail;
    lockout;
    constructor(prisma, mail, lockout = new login_lockout_service_1.LoginLockoutService()) {
        this.prisma = prisma;
        this.mail = mail;
        this.lockout = lockout;
    }
    cookieNames = { access: ACCESS_COOKIE, refresh: REFRESH_COOKIE };
    async register(input, response) {
        const email = input.email.trim().toLowerCase();
        if (!input.name.trim() || !email || input.password.length < 8) {
            throw new common_1.BadRequestException("Name, valid email, and password with at least 8 characters are required.");
        }
    ...
    async deleteAccount(accessToken, password, response) {
        const payload = this.verifyAccessToken(accessToken);
    ...
    async resetPassword(input) {
        if (!input.token || input.password.length < 8) {
            throw new common_1.BadRequestException("Reset token and password with at least 8 characters are required.");
        }
        const reset = await this.prisma.passwordReset.findUnique({ where: { tokenHash: this.hashToken(input.token) } });
        if (!reset || reset.usedAt || reset.expiresAt < new Date()) {
            throw new common_1.BadRequestException("Password reset token is invalid or expired.");
        }
        await this.prisma.$transaction([
            this.prisma.user.update({
    ...
    async verifyEmail(input) {
        ...
        if (!code) {
            throw new common_1.BadRequestException("Email verification token is invalid or expired.");
        }
        await this.prisma.$transaction([
    ...
    async issueSession(userId, response) {
        ...
        response.cookie(csrf_middleware_1.csrfCookieName, this.makeOpaqueToken(), {
            httpOnly: false,
            secure,
            sameSite: "lax",
            path: "/",
    ...
    clearAuthCookies(response) {
        ...
        response.clearCookie(csrf_middleware_1.csrfCookieName, { path: "/" });
    }
    async toAuthUser(userId) {
    ...
    accessSecrets() {
        return [this.accessSecret(), process.env.JWT_PREVIOUS_ACCESS_SECRET].filter(Boolean);
    }
    refreshSecrets() {
        return [this.refreshSecret(), process.env.JWT_PREVIOUS_REFRESH_SECRET].filter(Boolean);
    }
    async assertNotLocked(email) {
        if (await this.lockout.isLocked(email)) {
            throw new common_1.UnauthorizedException("Too many failed login attempts. Try again later.");
        }
    }
