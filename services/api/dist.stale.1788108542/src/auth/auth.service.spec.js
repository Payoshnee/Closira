...
describe("AuthService security flows", () => {
    ...
    it("locks login after repeated bad passwords", async () => {
        ...
        const service = new auth_service_1.AuthService(prisma, mail);
        await expect(service.login({ email: "user@example.com", password: "wrong-password" }, response)).rejects.toBeInstanceOf(common_1.UnauthorizedException);
        await expect(service.login({ email: "user@example.com", password: "wrong-password" }, response)).rejects.toBeInstanceOf(common_1.UnauthorizedException);
